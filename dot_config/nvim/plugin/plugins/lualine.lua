...
local function mode() return " " .. require("lualine.utils.mode").get_mode() end
...
local function resolve_duplicate_labels(hmap)
    ...
    for _, items in pairs(hmap) do
        if #items > 1 then return resolve_duplicate_labels(hmap) end
    end
    return hmap
end
...
local function harpoon_files()
    ...
    for index = 1, harpoon:list():length() do
        ...
        if #fpath == 0 then goto continue end
    ...
    local items = vim.tbl_map(function(item)
        ...
        if current_fpath == fullpath then
            ...
            return string.format(
                "%%#HarpoonNumberActive#  %d. %%#HarpoonActive#%s  ",
                index,
                label
            )
        else
            ...
            return string.format(
                "%%#HarpoonNumberInactive#  %d. %%#HarpoonInactive#%s  ",
                index,
                label
            )
        end
