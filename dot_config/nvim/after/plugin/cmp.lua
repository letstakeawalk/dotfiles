...
local len = require("HRB.utils").len
...
cmp.setup({
	...
	mapping = {
		...
		["<S-Tab>"] = cmp.mapping(function(fallback)
			-- select prev suggestion if visible, jump backward if in snippet, or fallback
			if cmp.visible() then
...
cmp.setup.cmdline(":", {
	mapping = {
		...
		["<Down>"] = {
			c = function(fallback) -- select next suggestion
				if cmp.visible() and len(cmp.get_entries()) > 1 then
		...
		["<Up>"] = {
			c = function(fallback) -- select prev suggestion
				if cmp.visible() and len(cmp.get_entries()) > 1 then
